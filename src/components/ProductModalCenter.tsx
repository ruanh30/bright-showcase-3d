...
interface ProductModalCenterProps {
  isOpen: boolean;
  onClose: () => void;
  product: ProductData | null;
}

const ProductModalCenter: FC<ProductModalCenterProps> = ({ isOpen, onClose, product }) => {
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto py-8 px-4">
        <div
          className="relative bg-card rounded-2xl w-full max-w-4xl animate-scale-in overflow-hidden"
          style={{ boxShadow: "0 30px 90px -20px hsla(0,0%,0%,0.25)" }}
          ...
          <div className="p-6 md:p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-5">
                <div>
                  <h2 className="font-display font-bold text-foreground text-2xl">{product.name}</h2>
            ...
            {/* Related */}
            <div className="mt-10 border-t border-border pt-6">
              <RelatedProducts products={product.relatedProducts} onProductClick={() => {}} />
            </div>
          </div>
        </div>
      </div>
...
export default ProductModalCenter;
